...
def render_tables_tab():
    ...
    def on_load_table(domain, table_name, limit):
        ...
        res = DBManager.get_table_data(clean_dir, clean_tbl, limit=int(limit))
        if res.get("error"):
        ...
        stats_text = f"✅ **Table `{res.get('domain', clean_dir)}.{res.get('table', clean_tbl)}`** — Displaying {len(data)} of {total} total rows.\nColumns: `{', '.join(cols)}`"
